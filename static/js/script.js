(function () {
  ...
  function get_route_2 (arrivalTime, delayTime2) {
   ...
   $.getJSON(url2, function(data2) {
        ...
        draw_route_2(data2);
        var arrivalTime = data2.plan.itineraries[0].endTime;
        var delayTime3 = $("#delayTime2").val();
        get_route_3(arrivalTime, delayTime3);
    });
  }

  function get_route_3 (arrivalTime, delayTime3) {
    if (delayTime3 === 'undefined'){
      delayTime3 = 0;
    }
    var url3 = generate_url_3(arrivalTime, delayTime3);
  ...
  function generate_url_2 (arrivalTime, delayTime2) {
    ...
    var pointA = markerList[1].getLatLng();
    var pointB = markerList[2].getLatLng();
    ...
    url2 = "http:localhost:8080/otp/routers/default/plan?fromPlace="+pointA.lat+"%2C"+pointA.lng+"&toPlace="+pointB.lat+"%2C"+pointB.lng+"&mode=TRANSIT%2CWALK&maxWalkDistance=750&arriveBy=false&date="+year+"-"+month+"-"+day+"&time="+hour+":"+min;
    // console.log(url2)
    return url2;
  }

  function generate_url_3 (arrivalTime, delayTime3) {
    if (delayTime3 === 'undefined'){
      delayTime3 = 0;
    }
    var pointA = markerList[2].getLatLng();
    var pointB = markerList[3].getLatLng();
    var inputTime = arrivalTime;

    var delay = parseInt(delayTime3) * 60 * 1000;
    var delayedStart = delay + inputTime;
    ...
    url3 = "http:localhost:8080/otp/routers/default/plan?fromPlace="+pointA.lat+"%2C"+pointA.lng+"&toPlace="+pointB.lat+"%2C"+pointB.lng+"&mode=TRANSIT%2CWALK&maxWalkDistance=750&arriveBy=false&date="+year+"-"+month+"-"+day+"&time="+hour+":"+min;
    // console.log(url2)
    return url3;
  }
  ...
  function draw_route_3 (data3) {
    var legs = data3.plan.itineraries[0].legs;
